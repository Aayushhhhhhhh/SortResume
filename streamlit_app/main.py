...
def _render_home():
    ...
    st.markdown("""
    <div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;
                letter-spacing:0.04em;margin-bottom:24px;">THE 4-SIGNAL ENGINE</div>
    """, unsafe_allow_html=True)
    ...
    for emoji, name, weight, color, desc in [
        ...
        ("💪","Substance Score","25%","#ffd166",
         "Measures if keywords are PROVEN with outcomes. 'Built Python pipeline saving 40%' scores 4x over just 'Python'."),
        ("🤖","AI Noise Detector","15%","#ff6b9d",
         "The feature nobody else has. Detects ChatGPT-generated padding using phrase analysis + linguistic signals."),
    ]:
        st.markdown(f"""
...
def _render_scorer():
    ...
    with col_left:
        st.markdown("""<div style="font-family:'DM Mono',monospace;font-size:10px;
                    ...
                    STEP 1 — UPLOAD RESUMES</div>""", unsafe_allow_html=True)
    ...
    submit = st.button("💎 SCORE ALL RESUMES", disabled=not can_submit, use_container_width=True)
    ...
    if submit and can_submit:
        ...
        successful = sorted(
            ...
            key=lambda x: x["result"].final_score, reverse=True
        )
                ...
                with right:
                    ...
                    def pills(items, color):
                        ...
                        return "".join(
                            ...
                            f'padding:3px 10px;margin:3px;font-size:12px;">{i}</span>'
                            for i in items
                        )

                    if res.matched_keywords:
                        st.markdown("**✅ Matched**")
                        st.markdown(f'<div style="margin:4px 0 12px">{pills(res.matched_keywords[:10],"green")}</div>', unsafe_allow_html=True)

                    if res.missing_required:
                        st.markdown("**❌ Missing Required**")
                        st.markdown(f'<div style="margin:4px 0 12px">{pills(res.missing_required[:8],"red")}</div>', unsafe_allow_html=True)

                    if res.top_missing_keywords:
                        st.markdown("**🔍 Top Missing Keywords**")
                        st.markdown(f'<div style="margin:4px 0 12px">{pills(res.top_missing_keywords[:8],"amber")}</div>', unsafe_allow_html=True)
        ...
        if successful:
            ...
            out = sio.StringIO()
            w = csv.writer(out)
            w.writerow(["Rank","File","Score","Tier","Semantic","Keyword","Substance","AI Noise","Missing Required","AI Generated"])
            for rank, r in enumerate(successful,1):
                res = r["result"]
                w.writerow([rank, r["filename"], res.final_score, res.tier_label,
                            res.semantic_score, res.keyword_score, res.substance_score,
                            res.ai_noise_score, " | ".join(res.missing_required[:5]),
                            "Yes" if res.is_likely_ai_generated else "No"])
            st.download_button("⬇️ DOWNLOAD CSV REPORT", out.getvalue(),
                             "sortresume_results.csv", "text/csv",
...
def _render_about():
    ...
    **The old signals are dead:**
    ...
    for name, weight, desc in [
        ("Semantic Match","35%","BAAI/bge-large-en-v1.5 embeddings. Understands meaning, not just words. Section-weighted — Experience matters more than a Skills list."),
        ("Keyword Match","25%","TF-IDF with bigrams. Section-aware weighting. Required JD skills penalise harder when missing."),
        ("Substance Score","25%","Measures if keywords are proven with outcomes and metrics. Level 0 (just listed) to Level 3 (quantified outcome with scale)."),
        ...
        with col1:
            st.markdown(f"""<div style="font-family:'Bebas Neue',sans-serif;
                        font-size:1.8rem;color:#00e5ff;text-align:center;">{weight}</div>""",
                        unsafe_allow_html=True)
        with col2:
